...
/* ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
   iAdministrator page ΓÇö Compliance Intelligence command center.
...
export function IAdministratorPage() {
  ...
  /* ΓöÇΓöÇ Submit handler ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  ...
  /* ΓöÇΓöÇ Pending mission handoff (returning-user mission prompt) ΓöÇΓöÇ
  ...
  /* ΓöÇΓöÇ Tab change re-runs the last command under the new intent ΓöÇΓöÇ */
  ...
  /* ΓöÇΓöÇ Action dispatch ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
    ...
    if (backendMode !== 'available' && backendMode !== 'index_not_built') {
      ...
      setRebuildError(
        backendMode === 'static_deploy'
          ...
          : 'Backend unavailable ΓÇö cannot rebuild index.',
      );
      return;
    }
  ...
  /* ΓöÇΓöÇ Phase-1 SSE: pre-load right panel the moment retrieval completes ΓöÇΓöÇ */
  useEffect(() => {
    if (demoCriticalState) return;
  ...
  /* ΓöÇΓöÇ Auto-select reference from first linkedReference / citation ΓöÇΓöÇ */
  useEffect(() => {
    if (demoCriticalState) return;
  ...
  return (
    <div className="w-full h-full flex flex-col" style={{ color: bgText }}>
      {/* ΓöÇΓöÇ Header + health strip ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <button
                ...
                title="Chat mode ΓÇö stateful case-based conversation"
              >
        ...
        <HealthStrip
          ...
          onRebuild={
            backendMode === 'available' || backendMode === 'index_not_built'
              ...
              : undefined   // disabled ΓÇö no backend in this deployment
          }
        />
        {rebuildState === 'running' && (
          <div className="mt-2 text-[11px] flex items-center gap-2" style={{ color: subtle, fontFamily: "'JetBrains Mono', monospace" }}>
            <Loader2 size={12} className="animate-spin" /> Rebuilding index ΓÇö this can take a few minutes on first run.
          </div>
        )}
        {rebuildError && (
      ...
      {/* ΓöÇΓöÇ Two-column workspace ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      <div className="flex-1 w-full px-6 md:px-8 py-4 md:py-6 overflow-hidden grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,420px)] xl:grid-cols-[minmax(0,1fr)_minmax(0,480px)] gap-3 md:gap-4">
        ...
        {/* ΓöÇΓöÇ CHAT MODE ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        {chatMode ? (
          <>
            {/* LEFT: Chat thread */}
          ...
          <>
            {/* ΓöÇΓöÇ QUERY MODE (existing behavior) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      ...
      {/* Meta footer ΓÇö timings + intent badge (only when we have a response). */}
      {query.response?.meta && (
        <div
      ...
      {/* Hidden executive proposal trigger ΓÇö double-click bottom-left corner */}
      <button
        type="button"
...
/* ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
   Response stack ΓÇö orchestrates the order sub-components render in.
   ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
...
function PendingState({ isLight, intent, retrieving }: { isLight: boolean; intent: IntentKind; retrieving: boolean }) {
  ...
  const stageMsg = retrieving
    ? 'Brad is retrieving corpus ΓÇª'
    : `Brad is generating ${label.toLowerCase()} ΓÇª`;
  return (
    <div
  ...
  return (
    ...
    >
      ...
      <p className="text-[14px] leading-relaxed" style={{ color: text }}>
        Ask Brad a compliance question or issue a command. Brad retrieves only from the internal Home Health corpus ΓÇö policies, procedures, forms, and appendices ΓÇö and returns a structured, citation-backed answer with actionable references.
      </p>
