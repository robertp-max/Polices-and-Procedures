# 07 â€” Failure & Restart Log (Brad 1.0 LIVE)
...
**Scenario:** S05 â€” VRAM / GPU data remanence across sessions
**Finding ID:** C-01
...
### Control Failures
- HIPAA Â§164.312(a)(1) Access Control â€” uncontrolled secondary read path
- HIPAA Â§164.312(c)(1) Integrity / (b) Audit Controls â€” no logging of cross-session memory reuse
...
**Scenario:** S13 â€” Secrets leakage in repo (`Builder/orbital-stage-443721-v1-99d78d776418.json`)
**Finding ID:** C-03
...
### PHI Exposure Risk
**Indirect â†’ potentially direct**, depending on the service account's scopes (which were not enumerated in this assessment).

### Control Failures
- HIPAA Â§164.308(a)(4) Information Access Mgmt
- HIPAA Â§164.312(d) Person/Entity Authentication
- HIPAA Â§164.308(b)(1) BAA â€” depends on the Google service used
- SOC 2 CC6.1, CC6.7, CC9.2

### Recommended Remediation (NOT APPLIED)
See [05 Â§5.7](./05-Hardening-Blueprint.md#57-secrets-management). Headline: treat the credential as compromised for planning; remove from working tree and git history; rotate; move to Vault/SOPS/keychain; add `gitleaks` pre-commit hook; confirm BAA scope of underlying Google service.
...
**Scenario:** S06 â€” Audit log tampering
**Finding ID:** C-04
...
- HIPAA Â§164.312(c)(1)/(c)(2) Integrity
- HIPAA Â§164.308(a)(1)(ii)(D) Information System Activity Review
- SOC 2 CC4.1, CC4.2, CC7.2, CC7.3

### Recommended Remediation (NOT APPLIED)
See [05 Â§5.12](./05-Hardening-Blueprint.md#512-logging--audit-integrity-critical-recommendation). Headline: WORM sink with object-lock, hash-chained batches, hourly chain root signed by offline HSM, continuous chain verifier with P1 alarm on break, 7-year retention.
...
**Scenario:** S31 â€” Approval workflow bypass (no two-person rule)
**Finding ID:** C-05

### Weakness
There is no enforced server-side two-person approval engine for PIP / corrective action / chart write-back / PHI export operations. Any chart-affecting action triggered by the operator executes under the operator's identity alone. A single click â€” by an authorized but mistaken user, or by a compromise of the operator's session â€” can effect a governed change with no second-pair-of-eyes interlock.
...
### Control Failures
- HIPAA Â§164.308(a)(3)(ii)(A) Authorization/Supervision
- HIPAA Â§164.308(a)(4) Information Access Mgmt
- HIPAA Â§164.312(c)(1) Integrity
- SOC 2 CC6.1, CC6.3, CC8.1

### Recommended Remediation (NOT APPLIED)
See [05 Â§5.5](./05-Hardening-Blueprint.md#55-local-service-architecture) and [05 Â§5.8](./05-Hardening-Blueprint.md#58-rbac--identity-recommended). Headline: write-broker with deterministic OPA policy gate; two-person rule enforced server-side with FIDO2 attestation; signed envelopes for write operations; LLM never executes â€” it proposes only.
