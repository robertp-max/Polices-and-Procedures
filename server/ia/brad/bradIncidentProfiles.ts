...
export function detectIncidentProfile(text: string, route: IncidentRoute): IncidentProfileId | null {
  const t = text ?? '';
  // Patient-originated sexual misconduct, split by tier (assault checked first so
  // physical contact always gets the trauma-informed response, not the script).
  if (SEXUAL_ASSAULT_RE.test(t)) return 'sexual_assault';
  if (SEXUAL_HARASSMENT_RE.test(t)) return 'sexual_harassment';
...
export function composeIncidentAnswer(profile: IncidentProfileId, text: string): string {
  switch (profile) {
    ...
    case 'sexual_harassment': {
      const ack =
        'I’m sorry that happened. You do not have to tolerate sexual comments, advances, requests, exposure, or pressure from a patient — being in someone’s home doesn’t change that.';
      const script =
        'Keep your response calm, clear, and professional. You can say:\n\n“Mr./Ms. ____, I’m here as your clinician/caregiver, and I need our interaction to stay professional. Please don’t make sexual comments or advances toward me. If it continues, I’ll need to end the visit and notify my supervisor.”';
      const escalate =
        'If the behavior keeps up — or if the patient touches you, blocks your exit, exposes themselves, threatens you, or you feel unsafe at any point — end the visit and leave immediately if you can do so safely. Call your supervisor right away, and if you’re in immediate danger, call 911.';
      const document =
        'As soon as you’re safe, report it to your supervisor and document objectively: what was said or done, the date, time, and location, who was present, how you responded, whether the visit was completed or ended, that you notified your supervisor, and whether you need a safety plan going forward.';
      const controls =
        'Your supervisor may add protections if it continues — a safety plan, a buddy visit, reassignment, a visit-boundary agreement, or a discharge review. You don’t have to keep handling this on your own.';
      return [ack, script, escalate, document, controls].join('\n\n');
    }
