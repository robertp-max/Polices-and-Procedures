...
const CARE_INDEED_LOGO = '/assets/logo-careindeed-orange.png';
const SYNTHETIC_BADGE_PHOTO =
  'data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22480%22 height=%22480%22 viewBox=%220 0 480 480%22%3E%3Crect width=%22480%22 height=%22480%22 fill=%22%23EEF4F3%22/%3E%3Ccircle cx=%22240%22 cy=%22184%22 r=%2276%22 fill=%22%23007970%22 opacity=%22.18%22/%3E%3Cpath d=%22M96 444c18-101 74-151 144-151s126 50 144 151%22 fill=%22%23007970%22 opacity=%22.18%22/%3E%3Ctext x=%22240%22 y=%22440%22 text-anchor=%22middle%22 font-family=%22Arial%22 font-size=%2228%22 font-weight=%22700%22 fill=%22%23004142%22%3ESYNTHETIC PREVIEW%3C/text%3E%3C/svg%3E';
...
function BadgeBuilderModal({
  ...
  const [photo, setPhoto] = useState<string | null>(SYNTHETIC_BADGE_PHOTO);
  ...
  const startCamera = async () => {
    ...
    setCameraReady(false);
    stopCamera();
    setPhoto(SYNTHETIC_BADGE_PHOTO);
    setCameraError('Static synthetic photo selected. No camera permission was requested.');
  };
  ...
  return (
    <div className="absolute inset-0 z-50 overflow-hidden bg-[#1F1C1B]/58 backdrop-blur-[2px]">
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="synthetic-badge-title"
        tabIndex={-1}
        className="absolute inset-0 grid h-full w-full grid-cols-1 gap-0 overflow-hidden bg-white lg:grid-cols-[minmax(0,1fr)_280px]"
      >
        <button
          type="button"
          onClick={() => {
        ...
        <div className="flex min-h-0 flex-col justify-center overflow-y-auto px-8 py-8">
          ...
          <h2 id="synthetic-badge-title" className="mt-3 font-montserrat text-3xl font-bold leading-tight text-[#007970]">Create a Synthetic Clinician Badge</h2>
          <p className="mt-4 font-roboto text-sm leading-relaxed text-[#52404B]">
            Your badge is the first piece of trust patients see at the door. Use the static synthetic photo, optionally preview a local image, or use the Care Indeed logo. Nothing is uploaded.
          </p>
            ...
            {cameraActive ? (
              <div className="space-y-3">
                <p className="font-roboto text-sm leading-relaxed text-[#52404B]">
                  Optional image preview is shown only in this browser session.
                </p>
                <button
                  type="button"
        ...
        {badgeGenerated && (
          <div className="pointer-events-none absolute inset-0 z-30 flex items-center justify-center overflow-hidden bg-white/94 backdrop-blur-[1px]">
            ...
            <motion.div
              ...
              initial={prefersReducedMotion ? false : { y: -520, rotate: -18, opacity: 0 }}
              animate={
            ...
            <motion.button
              ...
              className="pointer-events-auto absolute bottom-8 left-1/2 min-h-11 -translate-x-1/2 rounded-full bg-[#004142] px-5 py-3 font-montserrat text-xs font-bold uppercase tracking-[0.16em] text-white shadow-[0_12px_30px_rgba(0,65,66,0.24)] focus-visible:outline focus-visible:outline-4 focus-visible:outline-offset-4 focus-visible:outline-[#F06923]"
            >
              Complete badge practice
            </motion.button>
          </div>
        )}
      </section>
...
export default function GAO001Scene01WelcomeDesk({
  onComplete,
  learnerName = 'Taylor Demo RN',
  learnerRole = 'Registered Nurse',
  learnerEmail = 'demo-rn-001@example.invalid',
}: GAO001Scene01WelcomeDeskProps) {
  ...
  const startCamera = async () => {
    setCameraActive(false);
    setBadgePhoto(SYNTHETIC_BADGE_PHOTO);
  ...
  useEffect(() => {
    if (document.getElementById('gao001-scene01-styles')) return;
    const style = document.createElement('style');
    style.id = 'gao001-scene01-styles';
