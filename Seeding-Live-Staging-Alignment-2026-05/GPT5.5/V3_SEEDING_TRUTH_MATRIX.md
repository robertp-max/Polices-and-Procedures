...
| CES Sprint Board | `/ui-staging/v32` section `ces` | `/ces/board`, `/calendar?view=sprint`, `/pm/*` | `src/ui-staging/V3_2StagingApp.tsx` | `V3_ExecutionUnitsSeed`, local Phase 4B preview state | `src/policy/ces/pages/CesBoardPage.tsx`, `src/policy/ces/components/board/SprintExecutionBoard.tsx`, `src/policy/pages/MasterCalendarPage.tsx`, `src/policy/components/pm/TaskDetailRightPanel.tsx` | Yes | Partial event/task content | No canonical PM drawer; V3 in-shell adapter | Yes, task card opens in-shell event/task detail | Yes, explicit `Open live route` only | Evidence/signature/approval local preview only | Yes, labeled | 4 (local preview only; not durable production execution) | Durable planner/task execution, backend evidence persistence, durable signature collection, approval mutation, certification, and audit-history mutation remain Phase 4C blockers. | `src/ui-staging/V3_2StagingApp.tsx`, `src/policy/ces/data/V3_CES_SeedData.ts` | Phase 4B is not level 5 production-shaped completion. |
