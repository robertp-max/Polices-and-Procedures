...
export const HELP_ARTICLES: Record<string, HelpArticle> = {
  ...
  /* === Community / Profile / Threads / Journey articles (Phase 1) === */

  'COMMUNITY-HOW-TO-USE': {
    id: 'COMMUNITY-HOW-TO-USE',
    title: 'How to use Community',
    subtitle: 'Staff hub for discussions and member directory (no PHI)',
    overview: 'Community is the internal no-PHI space where Care Indeed staff can find discussions, view colleague profiles (subject to privacy), and see training achievements.',
    purpose: 'Guide staff on participating in Community without exposing protected information.',
    whenRequired: 'Any time you want to browse team discussions, find colleagues, or review your own achievements.',
    responsible: 'All staff. Discussions are visible to authenticated Care Indeed employees.',
    steps: [
      { label: 'Open Community', detail: 'Use the Community item in navigation or /community.' },
      { label: 'Browse recent threads', detail: 'See real discussions from the Help Center thread store. Click any to read or reply.' },
      { label: 'View the member directory', detail: 'See names, roles, and visibility status. Click View profile for details.' },
      { label: 'Start a discussion', detail: 'Use Start thread — it opens the Help Center composer. Write operational questions only.' },
      { label: 'Respect privacy', detail: 'Some profiles are Private. You will see a limited view unless you are the owner or an admin.' },
    ],
    formsRequired: [],
    outputs: ['Access to internal discussions and directory.'],
    commonMistakes: ['Posting patient details or PHI — never do this. Threads are scanned and blocked if PHI is detected.'],
    auditTips: ['All thread activity is auditable. Profile visibility changes are logged in the browser for Phase 1.'],
    relatedPolicies: [],
    relatedEventIds: [],
    estimatedMinutes: 3,
  ...
  'THREADS-HOW-TO-USE': {
    id: 'THREADS-HOW-TO-USE',
    title: 'How to use Threads',
    subtitle: 'Ask questions and collaborate in Help Center discussions',
    overview: 'Threads are the discussion layer inside Help Center and Community. Use them for operational questions, Brad follow-ups, feature ideas, and workflow help.',
    purpose: 'Show how to start, reply, and follow threads safely.',
    whenRequired: 'When you have a question that others may benefit from or when following up on Brad guidance.',
    responsible: 'Any authenticated staff member.',
    steps: [
      { label: 'Start a thread', detail: 'From Help Center or Community, click Start a thread. Choose category and write a clear title + body (no PHI).' },
      { label: 'Reply and upvote', detail: 'Read existing threads, add replies, and upvote helpful posts.' },
      { label: 'Publish private Brad threads', detail: 'Threads started from Brad responses begin private to you. Use Publish to Help Center when ready to share.' },
      { label: 'Follow status', detail: 'Threads can be answered, accepted, merged, or archived by admins.' },
    ],
    formsRequired: [],
    outputs: ['Open discussion that can become a Help article or improvement.'],
    commonMistakes: ['Including patient identifiers — the PHI guard will block the post.'],
    auditTips: ['Thread content, authors, and timestamps are retained for compliance review.'],
    relatedPolicies: [],
    relatedEventIds: [],
    estimatedMinutes: 4,
    updatedAt: '2026-06-30',
  ...
  'PROFILE-VISIBILITY': {
    ...
    title: 'Profile visibility: Private vs Public',
    subtitle: 'Control who sees your full profile page',
    overview: 'Your Community profile page has a visibility setting. Private limits viewers to you and admins. Public allows any authenticated Care Indeed employee to view your basic profile.',
    purpose: 'Explain the two states and their effect.',
    whenRequired: 'When editing your profile or viewing others.',
    responsible: 'Profile owner and administrators.',
    steps: [
      { label: 'Go to your profile', detail: 'Personal Ops (me) → Personal profile, or /personal/profile.' },
      { label: 'Toggle Private / Public', detail: 'Only you and admins can change it. Changes persist in your browser for Phase 1.' },
      { label: 'What changes', detail: 'Private hides most of the page from non-admins. Public makes basic identity visible.' },
    ],
    formsRequired: [],
    outputs: ['Clear understanding of who can see what.'],
    commonMistakes: ['Assuming thread posts are also hidden — they are not. Thread content is always visible internally.'],
    auditTips: ['Visibility is a profile-shell control only.'],
    relatedPolicies: [],
    relatedEventIds: [],
    estimatedMinutes: 2,
    updatedAt: '2026-06-30',
  ...
  'THREADS-PUBLIC-INTERNALLY': {
    id: 'THREADS-PUBLIC-INTERNALLY',
    title: 'Why thread posts are public internally',
    subtitle: 'Threads are team knowledge — profile privacy does not apply',
    overview: 'Thread posts (questions, replies, Brad responses) are deliberately visible to authenticated internal employees. This allows the whole team to benefit from shared knowledge even if some profiles are private.',
    purpose: 'Clarify the intentional separation between profile privacy and thread visibility.',
    whenRequired: 'When staff wonder why they can read a thread from someone whose profile is private.',
    responsible: 'All staff.',
    steps: [
      { label: 'Understand the rule', detail: 'Profile visibility controls the profile page shell only.' },
      { label: 'Threads stay visible', detail: 'Started by, replies, upvotes, and content remain readable by Care Indeed employees.' },
      { label: 'No PHI ever', detail: 'The PHI guard prevents unsafe content from being saved in the first place.' },
    ],
    formsRequired: [],
    outputs: ['Trust that discussions are appropriately open for internal use.'],
    commonMistakes: ['Treating threads like direct messages or patient notes.'],
    auditTips: ['This design is intentional for team learning and documented in code and Help.'],
    relatedPolicies: [],
    relatedEventIds: [],
    estimatedMinutes: 2,
    updatedAt: '2026-06-30',
  ...
  'NO-PHI-COMMUNITY-THREADS': {
    id: 'NO-PHI-COMMUNITY-THREADS',
    title: 'No PHI in Community and Threads',
    subtitle: 'Strict rule and technical guardrails',
    overview: 'Community and Threads are operational discussion spaces. They must never contain protected health information.',
    purpose: 'Reinforce the rule and explain protections.',
    whenRequired: 'Before posting anything in Community or Help threads.',
    responsible: 'Every user who posts.',
    steps: [
      { label: 'Never include', detail: 'Patient names, MRNs, dates of birth, addresses, diagnoses, photos of records, or any direct identifiers.' },
      { label: 'PHI guard', detail: 'Posts are scanned before saving. High-confidence PHI blocks the write (with sanitize option for some flows).' },
      { label: 'When in doubt', detail: 'Describe the process or policy question generically. Link policies instead of patient data.' },
    ],
    formsRequired: [],
    outputs: ['Safe, compliant team knowledge base.'],
    commonMistakes: ['Using real client examples “just this once.”'],
    auditTips: ['Violations are detectable in thread history and audit logs.'],
    relatedPolicies: [{ id: 'HIPAA', label: 'Privacy & Security policies' }],
    relatedEventIds: [],
    estimatedMinutes: 3,
    updatedAt: '2026-06-30',
  },
  ...
  'JOURNEY-ACHIEVEMENTS-PROFILE': {
    id: 'JOURNEY-ACHIEVEMENTS-PROFILE',
    title: 'Journey achievements on your profile',
    subtitle: 'Positive training completions can appear on your Community profile',
    overview: 'When Journey training (ACHC, onboarding, etc.) is completed, positive achievements may surface in your profile Credentials tab. Only verified completions are shown.',
    purpose: 'Explain what appears and why nothing negative is shown.',
    whenRequired: 'After completing Journey modules or when reviewing your profile.',
    responsible: 'Learners and their supervisors.',
    steps: [
      { label: 'Complete training', detail: 'Finish required modules, quizzes, and gates in Journey.' },
      { label: 'View on profile', detail: 'Open your Personal profile → Credentials tab. Positive items like ACHC Annual Training Complete or Onboarding Cleared appear when mapped.' },
      { label: 'No deficiencies shown', detail: 'Incomplete, failed, or remediation items never appear on profiles.' },
    ],
    formsRequired: [],
    outputs: ['Recognition of real completed training milestones.'],
    commonMistakes: ['Expecting every module or every employee to show badges immediately (identity mapping is still evolving).'],
    auditTips: ['Achievements are derived read-only from Journey evidence and attempts.'],
    relatedPolicies: [],
    relatedEventIds: [],
    estimatedMinutes: 2,
    updatedAt: '2026-06-30',
  ...
  'STAFF-COMMENDATIONS-BADGES': {
    id: 'STAFF-COMMENDATIONS-BADGES',
    title: 'Staff commendations and badges',
    subtitle: 'Positive recognition with real sources only',
    overview: 'Badges recognize helpful contributions, Journey milestones, and peer support. Every badge has a documented real source. No automated performance scoring or patient-outcome badges exist.',
    purpose: 'Describe the Phase 1 recognition system and privacy controls.',
    whenRequired: 'When giving or receiving recognition or viewing badges on profiles.',
    responsible: 'All staff (peer commendations); admins for review.',
    steps: [
      { label: 'Sources', detail: 'journey_completion, thread_helpful (accepted answers), peer_commendation, admin_award.' },
      { label: 'Positive only', detail: 'No deficiencies, discipline, or clinical ranking badges.' },
      { label: 'Hide from public', detail: 'You can hide individual badges from your public profile view.' },
      { label: 'Moderation', detail: 'Admins can review and remove inappropriate peer commendations.' },
    ],
    formsRequired: [],
    outputs: ['Transparent, auditable recognition.'],
    commonMistakes: ['Treating badges as performance rankings.'],
    auditTips: ['Every badge records its source and date.'],
    relatedPolicies: [],
    relatedEventIds: [],
    estimatedMinutes: 3,
    updatedAt: '2026-06-30',
