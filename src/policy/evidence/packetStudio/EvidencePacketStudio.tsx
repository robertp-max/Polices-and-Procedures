...
export function EvidencePacketStudio() {
  ...
  return (
    <section className="grid gap-xl" data-hash-id="evidence-packet-studio" data-route="/evidence/packet-studio" data-template="evidence">
      ...
      <section className="rounded-lg border border-hairline bg-surface-glass p-xl shadow-rest">
        <div className="flex flex-col gap-lg desktop:flex-row desktop:items-start desktop:justify-between">
          ...
          <div className="grid min-w-[260px] gap-sm rounded-lg border border-card bg-tone-slate-bg p-lg text-xs text-secondary">
            ...
            <div className="flex items-center justify-between"><span>Packet status</span><ToneBadge status={draft.packetStatus.includes('Validated') || draft.packetStatus.includes('Accepted') ? 'validated' : 'review-required'} /></div>
      ...
      <div className="flex max-w-full items-end overflow-x-auto border-b border-hairline font-montserrat" role="tablist" aria-label="Evidence Packet Studio steps">
        {steps.map((step, index) => (
          <button
            ...
            className={`${workspaceStackedTabClass} -mb-px min-w-[150px] text-left ${
              selectedStep === index ? workspaceTabActiveClass : workspaceTabInactiveClass
            }`}
          >
            ...
            <div className="mt-lg grid gap-lg desktop:grid-cols-[140px_minmax(0,1fr)]">
              ...
              <div className="min-h-[620px] rounded-lg border border-card bg-white p-xl text-[#1f1c1b] shadow-rest">
                ...
                <div className="mt-xl grid gap-md text-sm">
                  ...
                  <p><strong>Packet status:</strong> {draft.packetStatus} (generated never equals complete)</p>
            ...
            <div className="mt-lg grid gap-md tablet-p:grid-cols-2">
              <div className="rounded-lg border border-card bg-tone-slate-bg p-lg">
                ...
                <p className="mt-sm text-sm font-light text-secondary">
                  {draft.sources.length - draft.missingSources.length} of {draft.sources.length} packet sources are available. {draft.packetStatus}. Missing child items keep task open (missing_form / missing_evidence / missing_signature / missing_minutes / unresolved_blocker / validation_failed).
                </p>
              </div>
              <div className="rounded-lg border border-card bg-tone-slate-bg p-lg">
