...
export function renderPacketDocument(
  ...
  const accentColors = profile.chrome.accentRail.colors;
  const primary = accentColors[0] ?? '#007c7a';
  const secondary = accentColors[1] ?? '#e87722';
  ...
  <style>
    ...
    *{box-sizing:border-box;}
    ...
    .accent-rail span:first-child{background:${escapeHtml(primary)};flex:3;}
    .accent-rail span:last-child{background:${escapeHtml(secondary)};flex:1;}
    ...
    .pg-h{font-size:22px;line-height:1.2;color:#1a3b40;margin:6px 0 0;break-after:avoid;page-break-after:avoid;}
    .rule{width:54px;height:3px;background:${escapeHtml(secondary)};margin:10px 0 16px;}
    ...
    .panel-title{font-size:12px;font-weight:700;color:${escapeHtml(primary)};text-transform:uppercase;letter-spacing:.04em;margin-bottom:10px;break-after:avoid;page-break-after:avoid;}
    .kv-row{display:flex;justify-content:space-between;gap:18px;border-bottom:1px solid #eef2f2;padding:5px 0;font-size:11.5px;}
    .kv-key{color:${escapeHtml(primary)};font-weight:600;}
    ...
    .data-table th{background:rgba(0,124,122,.06);color:#1a3b40;text-align:left;padding:7px 9px;border-bottom:2px solid rgba(0,124,122,.12);}
    ...
    .notice{border:1px solid #d8e7e7;background:#f8fbfb;border-left:4px solid ${escapeHtml(primary)};border-radius:8px;padding:10px 14px;font-size:11px;color:#335154;margin-bottom:12px;break-inside:avoid;page-break-inside:avoid;}
    .notice-warning{background:#fff7ed;border-color:#f3c4a8;border-left-color:${escapeHtml(secondary)};color:#9a3412;}
    ...
    .signature-role{font-size:10px;font-weight:700;color:${escapeHtml(primary)};text-transform:uppercase;margin-top:4px;}
...
function renderLogo(profile: PacketRenderingProfile): string {
  ...
  return `<svg class="ci-mark" viewBox="0 0 360 70" xmlns="http://www.w3.org/2000/svg" aria-label="${escapeHtml(profile.chrome.logo.altText)}"><g fill="none" stroke="#e87722" stroke-width="7" stroke-linecap="round"><circle cx="34" cy="22" r="13"/><path d="M8 56c0-16 12-26 26-26s26 10 26 26"/></g><text x="74" y="34" font-family="Roboto,Segoe UI,sans-serif" font-size="30" font-weight="700" fill="#1f2937">Care</text><text x="150" y="34" font-family="Roboto,Segoe UI,sans-serif" font-size="30" font-weight="700" fill="#007c7a">Indeed</text><text x="74" y="52" font-family="Roboto,Segoe UI,sans-serif" font-size="11" letter-spacing="2" fill="#7a7470">THE HEART OF HOME HEALTH</text></svg>`;
}
