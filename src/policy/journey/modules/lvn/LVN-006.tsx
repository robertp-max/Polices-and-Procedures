...
// ─── MAIN COMPONENT ──────────────────────────────────────────────────────────
const LVN006MedicationManagement: React.FC = () => {
      ...
      >
          ...
          {quizMode && (
            <div>
              ...
              <p style={{ margin: '0 0 8px', color: THEME.muted, fontSize: 14 }}>
                ...
                <strong>knowledge only</strong>. Practical medication competency requires observed
                demonstration, skills check-off, and authorized sign-off per agency policy—separate
              ...
              {!submitted ? (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 10,
                    border: 'none',
                    background: answeredCount < totalQ ? '#FCA5A5' : THEME.primary,
                    color: THEME.white,
                    fontWeight: 700,
                ...
                >
                  ...
                  <p style={{ margin: '8px 0 0', fontSize: 13.5, lineHeight: 1.5 }}>
                    {passed
                      ? 'You met the knowledge threshold for LVN-006. This does not by itself document practical medication competency. Complete observed skills demonstration and authorized check-off per agency policy, and continue to practice under RN direction within LVN scope and the Plan of Care.'
                      : `You need ${passCount}/${totalQ} (${MODULE_META.passing}%) to pass. Review rationales, retry the quiz, and re-read Six Rights, reconciliation, high-alert holds, teach-back, and error response.`}
                  </p>
        ...
        {!quizMode && page && Scene && (
          ...
          >
            ...
            <LVNNarrationFooter
        currentIndex={quizMode ? PAGES.length - 1 : pageIndex}
        total={PAGES.length}
        onPrevious={goPrev}
        previousDisabled={!quizMode && pageIndex === 0}
        onNext={quizMode ? () => setQuizMode(false) : goNext}
        nextLabel={!quizMode ? (pageIndex < PAGES.length - 1 ? 'Next Lesson →' : 'Start Knowledge Check →') : 'Back to Content'}
        centerLabel={quizMode ? 'Knowledge Check' : 'Lesson ' + (pageIndex + 1) + ' of ' + PAGES.length}
      />
    </div>
  );
};

export default LVN006MedicationManagement;
