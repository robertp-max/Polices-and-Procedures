...
function instanceFromTemplate(template: EventTemplate, date: Date): RegulatoryEvent {
  ...
  if (template.id === 'TPL-QA-MONTHLY-QAPI') {
    ...
    decorated.scopeType = 'previous_calendar_month';
    decorated.reportingPeriodStart = toISO(prevMonth);
    decorated.reportingPeriodEnd = toISO(prevEnd);
    decorated.executionWindowStart = iso;
    decorated.executionWindowEnd = iso;
    decorated.scheduledDate = iso;
    decorated.preferredScheduleRule = template.preferredScheduleRule || 'first Friday of the month at 10:00 AM';
    decorated.scopeLabel = `Previous calendar month (${toISO(prevMonth)} to ${toISO(prevEnd)})`;
    ...
    decorated.scopeType = 'previous_calendar_quarter';
    decorated.reportingPeriodStart = toISO(prevStart);
    decorated.reportingPeriodEnd = toISO(prevEnd);
    decorated.executionWindowStart = iso;
    decorated.executionWindowEnd = iso;
    decorated.scheduledDate = iso;
    decorated.preferredScheduleRule = template.preferredScheduleRule || 'second Friday of Q anchor months at 10:00 AM';
    decorated.scopeLabel = `Previous calendar quarter (${toISO(prevStart)} to ${toISO(prevEnd)})`;
  }
