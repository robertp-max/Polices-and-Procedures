...
function TravelightCanvas() {
  ...
  useEffect(() => {
    ...
    const gen = (forceDir: string | null = null) => {
      ...
      return { x, y, dir, speed: (1.5 + Math.random() * 2) * 2.1613, opacity: 0.077, history: [] as any[], isHeadActive: true, deathTime: null as number | null }
    }
    ...
    const draw = () => {
      ...
      for (let i = streaksRef.current.length - 1; i >= 0; i--) {
        ...
        s.history = s.history.filter((p: any) => now - p.time < STAY + FADE)
    ...
    <div data-shell-bg="" aria-hidden="true" style={{ position: 'fixed', inset: 0, zIndex: 0, pointerEvents: 'none', overflow: 'hidden', transform: 'scale(1.0777)', transformOrigin: 'center center', filter: 'brightness(0.9223)' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
        <div style={{ position: 'absolute', left: '75%', top: '75%', transform: 'translate(-50%,-50%)', width: 0, height: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0.03, mixBlendMode: 'screen' as const }}>
          <img src={`${import.meta.env.BASE_URL}logomark-white.svg`} alt="" style={{ maxWidth: 'none', width: '104.75vw', height: '104.75vw', animation: 'tlRotate 350s linear infinite', willChange: 'transform' }} onError={e => { (e.target as HTMLImageElement).style.display = 'none' }} />
        </div>
      </div>
      {Array.from({ length: 18 }, (_, i) => (
        <div key={i} style={{
          ...
          animation: `tlFloat ${20 + Math.random() * 20}s infinite cubic-bezier(0.45,0,0.55,1) ${Math.random() * -30}s`,
        }} />
      ))}
    </div>
  )
