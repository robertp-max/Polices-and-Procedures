...
import { Bookmark, Heart, Info, Share2, X, MessageSquare, HelpCircle, User, Settings, LayoutGrid, ClipboardCheck, Network, GraduationCap } from 'lucide-react';
...
export function V6Shell() {
  ...
  const isPolicyDetailRoute = pathname.startsWith('/library/') && pathname !== '/library' && pathname !== '/library/policies' && !pathname.includes('/print');
    ...
    <div className={cx('theme-ci-light-orange flex h-screen overflow-hidden font-light text-ink p-0 m-0 border-0 bg-canvas', isLessonPlayerRoute ? 'bg-surface-glass backdrop-blur-md shadow-glass-inset' : '')}>
        ...
        <>
          {showDock && (
            <button
              ...
              className={cx(
                'fixed left-5 top-5 z-popover grid h-10 w-10 place-items-center rounded-full bg-transparent text-ink shadow-none transition duration-300 ease-standard hover:-translate-y-0.5 hover:text-brand-teal-deep',
          ...
          <button
            ...
            className={cx(
              'group fixed right-5 top-5 z-popover flex h-11 w-11 items-center justify-center bg-transparent shadow-none transition duration-300 ease-standard hover:-translate-y-0.5 hover:text-brand-teal-deep',
              isPersonalOpsOpen ? 'text-brand-teal' : 'text-slate-400',
            )}
          >
            <div className="transition-all duration-300 group-hover:scale-110 group-hover:-translate-y-1 flex items-center justify-center">
              <User className="h-[22px] w-[22px]" strokeWidth={1.5} />
            </div>
            <span className="pointer-events-none absolute right-14 whitespace-nowrap opacity-0 transition-all duration-300 ease-out group-hover:-translate-x-2 group-hover:opacity-100 text-brand-teal font-medium z-10">
              Profile
            </span>
          </button>
          {showDock && (
      ...
      {showRouteChrome && (
        <button
          ...
          className={cx(
            'group fixed bottom-5 left-5 z-popover flex h-11 w-11 items-center justify-center bg-transparent shadow-none transition duration-300 ease-standard hover:-translate-y-0.5 hover:text-brand-teal-deep',
            activeNavItem === 'admin' ? 'text-brand-teal' : 'text-slate-400'
          )}
        >
          <div className="transition-all duration-300 group-hover:scale-110 group-hover:-translate-y-1 flex items-center justify-center">
            <Settings className="h-[22px] w-[22px]" strokeWidth={1.5} />
          </div>
          <span className="pointer-events-none absolute left-14 whitespace-nowrap opacity-0 transition-all duration-300 ease-out group-hover:translate-x-2 group-hover:opacity-100 text-brand-teal font-medium z-10">
            Admin
          </span>
        </button>
      )}
    ...
    >
      {items.map((item) => (
        ...
        >
          ...
          <span className="pointer-events-none absolute left-12 whitespace-nowrap opacity-0 transition-all duration-300 ease-out group-hover:translate-x-2 group-hover:opacity-100 text-brand-teal font-medium z-10">
            {item.label}
          </span>
        </button>
      ))}
  ...
  return (
    <>
      <button
        ...
        title="Feedback"
        className={cx(
          'group fixed right-6 top-1/2 -translate-y-1/2 z-[50] flex h-11 w-11 items-center justify-center bg-transparent shadow-none transition-all duration-300 hover:text-brand-teal-deep',
          feedbackPanelVisible ? 'text-brand-teal' : 'text-slate-400',
