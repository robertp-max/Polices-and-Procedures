...
// Resolve to real generated workflows (correct titles, policyRefs via resolver, forms, roles, triggers, linked/dependencies).
// Use canonical 6 IDs for prototype library density while ensuring 100% resolution to WORKFLOWS.
const LIBRARY_WORKFLOW_IDS = ['QA-WF-03', 'CO-WF-02', 'GV-WF-01', 'HR-WF-05', 'RM-WF-04', 'CL-WF-08'] as const;

function toRealWorkflowRow(id: string): WorkflowRow {
  const wf: any = WORKFLOWS[id];
  if (!wf) {
    // fallback never reached for valid ids
    return { domain: '—', domainOwner: '—', frequency: '—', risk: 'Medium', status: 'active', title: id, workflowId: id };
  }
  const cad = wf.cadence || {};
  const freqRaw = cad.interval || 'quarterly';
  const frequency = typeof freqRaw === 'string' ? (freqRaw.charAt(0).toUpperCase() + freqRaw.slice(1)) : 'Quarterly';
...
export const workflowRows: readonly WorkflowRow[] = LIBRARY_WORKFLOW_IDS.map(toRealWorkflowRow);

const workflowMetrics: readonly MetricTileData[] = [
  { label: 'Workflows', value: String(workflowRows.length), helper: 'Generated + resolved', tone: 'teal' },
