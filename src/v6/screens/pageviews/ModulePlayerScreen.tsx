...
function MediaSlot({ appLocation, sceneTitle, className = '' }: { appLocation: string; sceneTitle?: string; className?: string }) {
  ...
  const isFinalGaoOverlay = /^GAO-(00[6-9]|01\d|02[0-7])\.lesson\.l\d+\.delivery$/i.test(appLocation);
  if (ready) {
      ...
      <div className={wrapperBase}>
        {isVideo ? (
          ...
          <img
            src={assetPath}
            alt={alt}
            className={`block h-full w-full ${isFinalGaoOverlay ? "object-contain object-center" : "object-cover"}`}
          />
        )}
      </div>
    );
