...
function loadResume(learnerId: string, assignmentId: string, casePack: CasePack): ResumePayload | null {
  ...
  return {
    ...
    activeTimeSeconds: resume.activeTimeSeconds ?? 0,
...
export default function TabletopSession({ caseId, mode, accessMode, onExit }: TabletopSessionProps) {
  ...
  const basePack = useMemo(() => resolveTabletopCasePack(caseId), [caseId]);
  const assignmentId = useMemo(() => (basePack ? assignmentIdFor(basePack.id) : ''), [basePack]);
  const resumed = useMemo(
    () => (basePack ? loadResume(learnerId, assignmentId, basePack) : null),
    [basePack, assignmentId, learnerId],
  );
  const elapsedSeconds = useActiveTime(resumed?.activeTimeSeconds ?? 0);

  const [attemptNumber, setAttemptNumber] = useState<number>(() => (assignmentId ? readDraft(learnerId, assignmentId)?.attemptNumber ?? 1 : 1));
  ...
  const persistDraft = useCallback((): boolean => {
    ...
    const saved = writeDraft(learnerId, {
      assignmentId,
      resume: {
        casePackId: casePack.id,
        phase,
  ...
  const persistOnUnmountRef = useRef(persistDraft);
  persistOnUnmountRef.current = persistDraft;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const result = await commitTabletopEvidence({
        ...
        accessMode,
        authenticatedSubjectId: learnerId,
      });
      if (cancelled) return;
      setDiagnostics(diag);
      setScore(sc);
  ...
  return (
    <div className="bs-root bs-session">
      ...
      <PrivilegedAccessBanner mode={isPrivilegedAccessMode(accessMode) ? accessMode : null} />
