...
import { isEligibleVoter, isVotingMember, recomputeQuorum } from './engine/groupState';
...
export default function FacilitatorConsole({ casePack, state, dispatch, onEndSession }: FacilitatorConsoleProps) {
  ...
  const motionParticipants = state.participants.filter(isEligibleVoter);
            ...
            <div className="bs-motion-builder">
              <div className="bs-motion-field">
                ...
                <select id="fc-mover" value={moverId} onChange={(e) => setMoverId(e.target.value)} disabled={controlsDisabled}>
                  ...
                  {motionParticipants.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
                </select>
              </div>
              <div className="bs-motion-field">
            ...
            <table>
              ...
              <tbody>
                {state.participants.map((p) => {
                  const votingMember = isVotingMember(p);
                  const barred = !votingMember || !p.present || p.recused || p.conflict;
                  ...
                  return (
                    <tr key={p.id}>
                      ...
                      <td>
                        {recorded
                          ? <span className={`bs-vote-cell ${recorded}`}>{recorded.replace('_', ' ')}</span>
                          : <span className="bs-vote-cell not_present">{votingMember ? 'no vote' : 'non-voting'}</span>}
                      </td>
                    </tr>
                  );
                })}
