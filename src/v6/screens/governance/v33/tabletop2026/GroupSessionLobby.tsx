...
export default function GroupSessionLobby({ casePacks = DEFAULT_CASE_PACKS, onStart }: GroupSessionLobbyProps) {
  ...
  const canStart = Boolean(selectedCasePack) && joinCode.trim().length > 0 && roster.length >= 2 && namedOk;
    ...
    const quorumRule: QuorumRule =
      quorumKind === 'majority_of_voting_members'
        ...
        : quorumKind === 'fixed_count'
          ? { kind: 'fixed_count', count: Math.max(1, Math.ceil(participants.length / 2)) }
          ...
          {casePacks.map((pack) => {
            ...
            return (
              ...
              >
                ...
                <footer>
                  <small>Pass standard: {pack.passScore} / 1000</small>
        ...
        <div className="bs-rail-card">
          ...
          <div className="bs-motion-field">
            ...
            <select id="gb-quorum-rule" value={quorumKind} onChange={(e) => setQuorumKind(e.target.value as QuorumRule['kind'])}>
              <option value="majority_of_voting_members">Simple majority of seated members</option>
        ...
        <div className="bs-rail-card">
          ...
          <div className="bs-contradiction">
            ...
            <div>
              ...
              <p>
                Quorum is computed from every name on this roster. Add only individuals your bylaws seat as voting
                directors — attendees such as a non-voting Secretary/Facilitator or Observer will still count toward
                the roster total if added here.
              </p>
            </div>
          </div>
