...
/** A participant is barred from voting on the current matter if absent, recused, or conflicted. */
function isEligibleVoter(p: Participant): boolean {
  return p.present && !p.recused && !p.conflict;
}
...
export function recomputeQuorum(state: GroupSessionState): QuorumStatus {
  const totalMembers = state.participants.length;
