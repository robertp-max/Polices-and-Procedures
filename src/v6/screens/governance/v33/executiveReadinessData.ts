...
export interface ReadinessDecision {
  ...
  referenceMaterials?: Array<{
    label: string;
    /**
     * Reference DESCRIPTOR only (blocker 6 / spec §6). Controlled documents are
     * never linked at a public static URL and are never bundled into client JS.
     * The descriptor resolves at click time to either the protected server
     * endpoint (/api/governance/references/:docId, opened in a new tab) or the
     * canonical Journey handbook player — never an in-Governance iframe.
     */
...
/**
 * Spec §3 — these two items were REMOVED from the formal Decisions docket.
 *
 * They are mandatory INDIVIDUAL COMPLIANCE REQUIREMENTS, not motions: there is
 * nothing for the Board to approve, vote on, or authorize resources for. They
 * belong in My Compliance and in the readiness calculation, and the Decisions
 * page instead shows a compact read-only Board Readiness summary that links to
 * member compliance. They are retained here (not deleted) so the underlying
 * requirement, authority, and evidence expectations remain on the record.
 *
 * The formal docket keeps only matters requiring review, approval, motion,
 * vote, conditions, documented acceptance, or resource authorization.
 */
export const GB_COMPLIANCE_OBLIGATIONS: ReadinessDecision[] = [
  {
    id: 'GB-READINESS-006',
...
export const READINESS_DECISIONS: ReadinessDecision[] = [
  ...
  },
  {
    ...
    referenceMaterials: [
      {
        label: 'Open Patient Admission Packet',
