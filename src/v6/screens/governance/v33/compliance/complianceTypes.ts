...
export interface ComplianceAssignment {
  ...
  /**
   * The unit `passStandard` is expressed in. Required whenever `passStandard`
   * is non-null so no surface can render a 1000-point standard as "950%".
   */
  passStandardScale: EvidenceScoreScale | null;
  attemptCount: number;
...
export interface ComplianceEvidenceRecord {
  ...
  /**
   * Marks an attempt produced under a PRIVILEGED preview tier (Super Admin or
   * UAT Reviewer). Optional so existing records/constructions are unchanged.
   * When set, the attempt is preview-only: `commitEvidence` refuses to save it
   * as official completion, so it can never satisfy a requirement, appear as an
   * official participant attempt, or advance the 30-day readiness calculation.
   */
  privilegedAccessMode?: 'superadmin' | 'uat_reviewer' | null;
...
export interface ComplianceCourseGroup {
  ...
  courseTitle: string;
  passStandard: number | null;
  /** Unit `passStandard` is expressed in (course assessments: percentage). */
  passStandardScale: EvidenceScoreScale | null;
  recurrence: string | null;
