...
import { isPrivilegedAccessMode } from './accessMode';
...
export function deriveUserFacingStatus(assignment: ComplianceAssignment, now = Date.now()): UserFacingStatus {
  ...
  const draft = readDraft(assignment.learnerId, assignment.assignmentId);
...
export function resolveAssignmentView(assignment: ComplianceAssignment, now = Date.now()): ComplianceAssignmentView {
  ...
  return {
    ...
    officiallyComplete,
    hasLocalDraft: readDraft(assignment.learnerId, assignment.assignmentId) !== null,
  };
}
