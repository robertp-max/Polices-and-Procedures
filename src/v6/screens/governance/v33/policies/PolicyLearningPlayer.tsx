  ...
  return <div className="pp-player">
    ...
    <main className="pp-canvas">
      ...
      {mode === 'result' && <article className={`pp-result-card ${passed ? 'passed' : 'remediation'}`}>
        <header><div className="pp-score-orbit"><strong>{score}%</strong><span>SCORE</span></div><div><span>ATTEMPT {attempt} · {earned}/{questions.length} CORRECT</span><h2>{passed ? 'Pass standard met.' : 'Remediation recommended.'}</h2><p>{passed ? 'Required for your Governing Body compliance. Completion requires a passing result, attestation, and an official evidence record. This developer preview uses auto-generated items and is not official assessment evidence; official scoring is available only through a reviewed question bank and a connected evidence service.' : `This attempt did not meet the ${activity.passScore}% pass standard. Review each missed item against its controlling section, then retry.`}</p></div></header>
