...
- **ACHC Crosswalk** (`id: 'achc-crosswalk'`, route: `/framework/achc-survey?view=crosswalk`, icon: `network`, template: `achc-crosswalk`)
...
- **General:** Static JS arrays (frameworkDomains, achcSurveyRowsSeed, achcCrosswalkRowsSeed). No useState for filters/search/selection. Shared components (MetricTile, SurfaceCard, ToneBadge, Icon) good but applied inconsistently (crosswalk avoids DataTable). Route hack (`/framework/achc-survey?view=crosswalk`). No deep links, no domain-to-filter wiring, minimal mobile adaptation in grid, Brad overlay interference noted in captures. No visual coverage indicators or traceability graphs.
