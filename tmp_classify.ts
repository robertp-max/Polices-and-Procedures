...
export function classifyScenario(input: string): ScenarioClassification | null {
  ...
  // Human-first distress router (before app data / corpus) ΓÇö staff safety / boundary / allegation cases
  ...
  if (distress) {
    return {
      ...
      excludedByTerms: [],
      emergencyTriggerExplanation: distress.id.includes('life') || distress.id.includes('assault') || distress.id.includes('boundary') ? 'Field staff safety / distress ΓÇö human supervisor response required first.' : null,
      score: 12,
      confidence: 'high',
    };
  }
